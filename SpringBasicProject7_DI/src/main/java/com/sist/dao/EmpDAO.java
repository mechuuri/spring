...
public class EmpDAO {
	 ...
	 * 
	 * 		  WepApplicationContext : MVC
	 * *** 5page => 최신의 스프링은 자바 기반의 환경설정을 많이 사용하고 있다
	 * 	   =====> 환경설정 파일 : 생성, 주입, 클래스 간의 연결 관계 설정 
	 * 								  ------------------- XML, 자바 => 스프링에서 읽어서 설정
	 * 								  ------------------- XML은 반드시 스프링에서 지원하는 태그와 속성만 사용가능 
	 * 	    객체 생성
	 * 		1. 클래스 한개씩 메모리 할당 <bean>
	 * 		2. 패키지 단위로 메모리 할당 (*****) <context:component-scan> : 클래스를 구분 (구분자로 어노테이션 사용) @Controller
